      ...
      <span class="actions-inline">
        ${it.type === "dir"
          ...
          : it.name.toLowerCase().endsWith(".zip")
            ? `<button data-act="unzip"   data-path="${escapeAttr(it.path)}" title="Extract ZIP into repo" style="color:var(--accent)">Unzip</button>
...
function wireFileListInteractions(data) {
  ...
  panelBody.querySelectorAll(".file-row .actions-inline button").forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      if (act === "unzip")   actionUnzipRepo(path);
    });
  });
  ...
  const updateCount = () => {
    state.selected.clear();
    // Track ALL checked items (files AND dirs) with type info for download expansion
    state.selectedItems = [];
    panelBody.querySelectorAll(".row-cb:checked").forEach(cb => {
      const item = { path: cb.dataset.path, type: cb.dataset.type };
      state.selectedItems.push(item);
      if (cb.dataset.type === "file") state.selected.add(cb.dataset.path);
    });
    const nAll   = state.selectedItems.length;
    // Count files + dirs for the ZIP button label
    const nDownloadable = nAll;
    ...
    if (dlBtn) {
      dlBtn.disabled = nDownloadable === 0;
      dlBtn.innerHTML = nDownloadable === 0
        ...
        : `<svg viewBox="0 0 24 24" width="14" height="14"><path fill="currentColor" d="M12 16l-5-5 1.4-1.4 2.6 2.6V4h2v8.2l2.6-2.6L17 11l-5 5zm-7 2h14v2H5z"/></svg> Save ZIP (${nDownloadable})`;
    }
  ...
  $("downloadZipBtn")?.addEventListener("click", () => {
    // Re-read from DOM at click time (more reliable than stale state.selectedItems)
    const items = Array.from(panelBody.querySelectorAll(".row-cb:checked"))
      .map(cb => ({ path: cb.dataset.path, type: cb.dataset.type }));
    if (!items.length) {
      toast("Select at least one file or folder first", "error");
      return;
    }
    actionDownloadZip(items);
  });
...
/* ============================================================
   DOWNLOAD SELECTED FILES AS ZIP — full folder structure preserved
   Accepts an array of {path, type} items.
   Directories are recursively expanded via /list-files before
   fetching, so the complete nested structure ends up in the ZIP.
   Files are fetched in parallel batches of 6 for speed.
   ============================================================ */

/**
 * Recursively list ALL file paths inside a repo directory.
 * Returns a flat array of file path strings.
 */
/**
 * Expand selected items into a flat file list.
 * Uses /list-tree (ONE API call) for dirs — fast, handles deep nesting.
 */
async function expandItemsToFiles(items, signal) {
  const seen   = new Set();
  const result = [];

  const selectedFiles = items.filter(it => it.type === "file");
  const selectedDirs  = items.filter(it => it.type === "dir");

  for (const it of selectedFiles) {
    if (!seen.has(it.path)) { seen.add(it.path); result.push(it.path); }
  }

  if (selectedDirs.length === 0) return result;

  // Fetch FULL recursive tree in one call
  if ($("pgStatus")) $("pgStatus").textContent = "Scanning repository tree…";
  let allTreeFiles = [];
  try {
    const treeData = await api("/list-tree", {}, { retries: 2, signal });
    allTreeFiles = (treeData.items || []).filter(it => it.type === "file");
  } catch (e) {
    if (e.name !== "AbortError") {
      const logEl = $("pgLog");
      if (logEl) logEl.innerHTML +=
        `<div style="color:var(--warning)">⚠ Could not fetch repo tree: ${escapeHtml(e.message)}</div>`;
    }
    return result;
  }
  ...
  for (const dir of selectedDirs) {
    ...
    for (const treeFile of allTreeFiles) {
      if (treeFile.path.startsWith(prefix)) {
        if (!seen.has(treeFile.path)) { seen.add(treeFile.path); result.push(treeFile.path); }
      }
    }
  }
  return result;
...
async function actionDownloadZip(items) {
  ...
  const zipName  = `${repoSlug}_${Date.now()}.zip`;
  const ctrl     = new AbortController();

  const hasDirs = items.some(it => it.type === "dir");

  setPanel(`Preparing ZIP download…`, `
    <div class="folder-summary">
      ${cell("Selected", items.length + " item(s)")}
      ${cell("Repo", escapeHtml(`${state.username}/${state.repo}`))}
      ${cell("Output", escapeHtml(zipName))}
      ${cell("Status", `<span id="pgStatus">${hasDirs ? "Scanning folders…" : "Fetching files…"}</span>`)}
    </div>
    <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px">
      <div class="progress-shell" style="flex:1"><div class="progress-fill" id="pgFill" style="width:0%"></div></div>
      <button class="ghost-btn" style="white-space:nowrap;font-size:12px"
        onclick="this.disabled=true;this.textContent='Cancelling…';" id="dlCancelBtn">Cancel</button>
    </div>
    <div id="pgLog" style="font-size:12px;color:var(--text-dim);margin-top:4px;max-height:300px;overflow-y:auto;padding:0 2px"></div>
  `);

  $("dlCancelBtn")?.addEventListener("click", () => ctrl.abort());

  // ── Phase 1: expand dirs → flat list of file paths ──────────────────
  let filePaths;
  try {
    filePaths = await expandItemsToFiles(items, ctrl.signal);
  } catch (e) {
    if ($("pgStatus")) $("pgStatus").innerHTML =
      `<span style="color:var(--danger)">✕ Scan failed: ${escapeHtml(e.message)}</span>`;
    toast("Could not scan folders: " + e.message, "error");
    return;
  }
  ...
  if (!filePaths.length) {
    toast("No files found in selection", "error");
    return;
  }

  if ($("pgStatus")) $("pgStatus").textContent = `Fetching ${filePaths.length} file(s)…`;

  // ── Phase 2: fetch all files in parallel batches ─────────────────────
  const zip    = new JSZip();
  const BATCH  = 6;   // parallel fetches
  let   done   = 0;
  let   errors = 0;
  const logEl  = () => $("pgLog");
  ...
  for (let i = 0; i < filePaths.length; i += BATCH) {
    if (ctrl.signal.aborted) {
      if ($("pgStatus")) $("pgStatus").innerHTML =
        `<span style="color:var(--warning)">⊘ Cancelled — ${done} file(s) fetched</span>`;
      return;
    }
    ...
    const results = await Promise.allSettled(
      batch.map(path =>
        api(`/read-file?path=${encodeURIComponent(path)}`, {}, { retries: 1, signal: ctrl.signal })
          .then(data => ({ path, file: data.file }))
      )
    );
    ...
    for (const r of results) {
      if (r.status === "fulfilled") {
        ...
        // zip.file(path, ...) preserves the full directory structure inside the ZIP
        if (f.is_binary) {
        ...
        if (logEl()) logEl().innerHTML +=
          `<div><span style="color:var(--success)">✓</span> ${escapeHtml(path)}</div>`;
      } else {
        if (r.reason?.name !== "AbortError") {
          errors++;
          const pathHint = batch[results.indexOf(r)] || "?";
          if (logEl()) logEl().innerHTML +=
            `<div><span style="color:var(--danger)">✕</span> ${escapeHtml(r.reason?.message || "error")}</div>`;
        }
      }
      done++;
    }
  ...
  try {
    ...
    if ($("pgStatus")) $("pgStatus").innerHTML =
      `<span style="color:var(--success)">✓ ZIP saved — ${done - errors} file(s)${errors ? `, ${errors} error(s)` : ""}</span>`;

    toast(
      errors > 0 ? `ZIP saved with ${errors} error(s)` : `ZIP saved: ${done - errors} file(s)`,
...
/**
 * Robust mobile folder path isolation.
 *
 * Problem: on some Android devices (especially Samsung / Chrome) the
 * webkitRelativePath includes the full storage path, e.g.
 *   storage/emulated/0/ps/file.txt   (wanted: ps/file.txt)
 *
 * Fix: find the DEEPEST common directory prefix across ALL files,
 * extract its last segment as the selected folder name, and strip
 * everything before it.  This is safe for normal paths too (prefix
 * length = 0 → nothing stripped).
 */
function resolveAndroidPaths(files) {
  const raws = files.map(f => (f.webkitRelativePath || "").replace(/^\/+/, ""));
  const withPath = raws.filter(r => r.includes("/"));
  if (!withPath.length) return { rootFolder: "", stripPrefix: "", raws };

  // Split each path into directory segments (exclude the filename itself)
  const segs = withPath.map(r => r.split("/").slice(0, -1));
  const first = segs[0];

  // Count how many leading segments are identical across ALL files
  let commonLen = 0;
  for (let i = 0; i < first.length; i++) {
    if (segs.every(s => s[i] === first[i])) commonLen++;
    else break;
  }

  if (commonLen === 0) {
    // No common prefix at all — treat first segment as root
    return { rootFolder: first[0] || "", stripPrefix: "", raws };
  }

  const commonParts = first.slice(0, commonLen);
  // The selected folder is the LAST common segment (e.g. "ps")
  const rootFolder = commonParts[commonParts.length - 1];
  // Everything BEFORE that last segment is storage junk to strip
  const stripParts  = commonParts.slice(0, -1);
  const stripPrefix = stripParts.length ? stripParts.join("/") + "/" : "";

  return { rootFolder, stripPrefix, raws };
}
...
function handleFilesPicked(e, isFolder) {
  ...
  const SKIP_SEGS = new Set([".git", "node_modules", "__pycache__"]);

  let rootFolder = "";
  let items;

  if (isFolder) {
    const { rootFolder: rf, stripPrefix, raws } = resolveAndroidPaths(files);
    rootFolder = rf;
    ...
    items = files.map((f, i) => {
      ...
      if (!rel) {
        // Very old Android — no relative path info, treat as flat file
        return { file: f, rel: f.name };
      }

      // Strip the device storage prefix (e.g. "storage/emulated/0/")
      if (stripPrefix && rel.startsWith(stripPrefix)) {
        rel = rel.slice(stripPrefix.length);
      }

      // ── STRICT ISOLATION ──
      // Only keep files whose path begins with the selected root folder.
    ...
    .filter(x => !x.rel.split("/").some(seg => SKIP_SEGS.has(seg)));

  } else {
    // Plain file upload — no folder structure, just filenames
  ...
  // For folder uploads: items[].rel currently includes the root folder name
  // e.g. "gitpush/app.py". We need to offer stripping it so files go to root directly.
  let workingItems = items;

  function getStrippedItems() {
    if (!isFolder || !rootFolder) return workingItems;
    const prefix = rootFolder + "/";
    return workingItems.map(it => ({
      ...it,
      rel: it.rel.startsWith(prefix) ? it.rel.slice(prefix.length) : it.rel,
    })).filter(it => it.rel);
  }

  function buildPreview(base, stripped) {
    const useItems = stripped ? getStrippedItems() : workingItems;
    const prefix   = base ? base.replace(/\/+$/, "") + "/" : "";
    return useItems.slice(0, 5).map(it =>
      `<div style="font-family:monospace;font-size:11px;color:var(--text-dim);padding:1px 0">
        ${escapeHtml(prefix + it.rel)}
      </div>`
    ).join("") + (useItems.length > 5
      ? `<div style="font-size:11px;color:var(--text-dim);padding:2px 0 0">…and ${useItems.length - 5} more</div>`
      : "");
  }

  const stripCheckedByDefault = isFolder && !!rootFolder;
  ...
  openModal({
    ...
    bodyHtml: `
      ...
      <div class="form-row" style="margin-bottom:10px">
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;user-select:none">
          <input type="checkbox" id="stripFolderName" ${stripCheckedByDefault ? "checked" : ""} style="width:16px;height:16px;cursor:pointer" />
          <span>Upload <b>contents</b> of <span class="user-tag">${escapeHtml(rootFolder)}</span> directly (don't create the folder in repo)</span>
        </label>
        <span class="hint">✓ = files go to root &nbsp;·&nbsp; ✗ = creates <b>${escapeHtml(rootFolder)}/</b> folder in repo</span>
      </div>` : ""}
      <div class="form-row">
        ...
        <input id="fBase" placeholder="leave empty for repo root" value="${escapeAttr(defaultBase)}" autocomplete="off" />
      </div>
      <div class="form-row">
        ...
        <div id="pathPreview" style="background:var(--surface-2,rgba(255,255,255,.04));border:1px solid var(--border);border-radius:6px;padding:8px 10px;min-height:32px">
          ${buildPreview(defaultBase, stripCheckedByDefault)}
        </div>
      </div>
      <div class="form-row">
        <label>Commit message</label>
    ...
    footer: [
      ...
      { label: `Upload ${items.length} file${items.length > 1 ? "s" : ""}`, className: "primary", onClick: (close) => {
          ...
          const stripped  = $("stripFolderName")?.checked ?? stripCheckedByDefault;
          const finalItems = stripped ? getStrippedItems() : workingItems;
          close();
          if (!isFolder && finalItems.length === 1) {
            runSingleFileUpload(finalItems[0], baseDir, msgPrefix);
          } else {
            runFolderUpload(finalItems, baseDir, msgPrefix);
          }
      }},
    ],
  });
  ...
  setTimeout(() => {
    const fBase   = $("fBase");
    const stripCb = $("stripFolderName");
    const preview = $("pathPreview");
    if (!fBase || !preview) return;
    const refresh = () => {
      const stripped = stripCb?.checked ?? stripCheckedByDefault;
      preview.innerHTML = buildPreview(fBase.value.trim().replace(/^\/+|\/+$/g, ""), stripped);
    };
...
const STREAM_THRESHOLD  = 2 * 1024 * 1024;   // 2 MB per file
// Total-batch threshold: switch to streaming if combined JSON payload is too large
const STREAM_TOTAL      = 5 * 1024 * 1024;   // 5 MB total payload
// File-count threshold: force streaming for large folders even if each file is tiny.
// This prevents a single massive JSON body on 1000+ file uploads.
const STREAM_COUNT      = 50;                 // > 50 files → always stream
const HARD_GITHUB_LIMIT = 100 * 1024 * 1024; // 100 MB GitHub REST maximum
...
async function runFolderUpload(items, baseDir, msgPrefix) {
  ...
  // Stream when any file is large, total payload is large, OR file count exceeds limit.
  // The count guard prevents a single giant JSON body for 1000+ tiny files.
  const useStream  = hasLarge || totalBytes > STREAM_TOTAL || total > STREAM_COUNT;
  const modeLabel  = useStream ? `Streaming (concurrent × 3, ${total} files)` : `Inline batch (${total} files)`;
...
/* ============================================================
   Safe file → ArrayBuffer helper
   Uses file.arrayBuffer() when available (modern browsers) and
   falls back to FileReader for older Android WebViews.
   ============================================================ */
function safeReadAsArrayBuffer(file) {
  if (typeof file.arrayBuffer === "function") {
    return file.arrayBuffer().catch(() => _readViaFileReader(file));
  }
  return _readViaFileReader(file);
}

function _readViaFileReader(file) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
...
function actionUploadZip() {
  ...
  picker.onchange = async (e) => {
    ...
    if (!file) return;
    // Android may not preserve extension — also check MIME type
    ...
    // Read ZIP entries — use safe reader with FileReader fallback for Android
    let zip;
    try {
      showLoader("Reading ZIP…");
      const buffer = await safeReadAsArrayBuffer(file);
...
async function runZipUpload(entries, baseDir, msgPrefix) {
  ...
  if (!filePayload.length) { toast("No files could be read from ZIP", "error"); return; }
  if (ctrl?.signal.aborted) {
    if ($("pgStatus")) $("pgStatus").innerHTML = `<span style="color:var(--warning)">⊘ Cancelled</span>`;
    toast("Upload cancelled", "error");
    return;
  }

  // Use streaming commit-tree path — works for any number/size of files
  // Convert base64 payloads back to Blobs and stream each as a blob
  if ($("pgStatus")) $("pgStatus").textContent = `Uploading ${filePayload.length} files to GitHub…`;
  if ($("pgFill"))   $("pgFill").style.width = "55%";

  const blobs = [];
  let uploadedCount = 0;

  for (const fp of filePayload) {
    if (ctrl?.signal.aborted) {
      if ($("pgStatus")) $("pgStatus").innerHTML = `<span style="color:var(--warning)">⊘ Cancelled at ${uploadedCount}/${filePayload.length}</span>`;
      toast("Upload cancelled", "error");
      return;
    }
    try {
      // Convert base64 back to binary blob for streaming
      const binaryStr = atob(fp.content);
      const bytes = new Uint8Array(binaryStr.length);
      for (let i = 0; i < binaryStr.length; i++) bytes[i] = binaryStr.charCodeAt(i);
      const blob = new Blob([bytes]);
      const file = new File([blob], fp.path.split("/").pop());
      const sha = await uploadBlobStreamXHR({ file, rel: fp.path }, null);
      blobs.push({ path: fp.path, sha });
    } catch (e) {
      const logEl = $("pgLog");
      if (logEl) logEl.innerHTML += `<div style="color:var(--danger)">✕ ${escapeHtml(fp.path)}: ${escapeHtml(e.message)}</div>`;
    }
    uploadedCount++;
    const pct = 55 + Math.round((uploadedCount / filePayload.length) * 35);
    if ($("pgFill")) $("pgFill").style.width = pct + "%";
    if ($("pgStatus")) $("pgStatus").textContent = `Uploading… ${uploadedCount}/${filePayload.length}`;
  }

  if (!blobs.length) { toast("No files could be uploaded", "error"); return; }
  ...
  try {
    const data = await api("/commit-tree", {
      method: "POST",
      body: JSON.stringify({ blobs, base_dir: baseDir, message: `${msgPrefix} (${blobs.length} files)` }),
    }, { retries: 1 });
...
/* ============================================================
   Unzip a ZIP that already lives in the repo
   Flow:
     1. Read the ZIP file content via /read-file (base64)
     2. Decode with JSZip in the browser
     3. Show modal: target dir + commit message + file preview
     4. Upload all extracted files via existing runFolderUpload()
   ============================================================ */
async function actionUnzipRepo(zipPath) {
  if (!ensureRepo()) return;
  if (typeof JSZip === "undefined") {
    toast("JSZip library not loaded — refresh the page.", "error");
    return;
  }

  // Step 1 — fetch ZIP bytes from GitHub via our backend
  let zipData;
  try {
    showLoader("Downloading ZIP from repo…");
    zipData = await api(`/read-file?path=${encodeURIComponent(zipPath)}`);
  } catch (err) {
    toast("Could not read ZIP: " + err.message, "error");
    return;
  } finally {
  ...
  // /read-file returns { ok:true, file: { encoding, content, is_binary, ... } }
  const fileInfo = zipData.file;
  if (!fileInfo) {
    toast("Unexpected response from server — could not read ZIP", "error");
    return;
  }

  // ZIP is always binary; backend returns base64-encoded content
  const rawB64 = fileInfo.content;
  if (!rawB64) {
    toast("ZIP file appears empty or unreadable", "error");
    return;
  }
  if (fileInfo.size > 1_000_000) {
    toast("Warning: ZIP is large (>1 MB). GitHub API may time out.", "");
  }

  // Step 2 — parse with JSZip
  let zip;
  try {
    showLoader("Parsing ZIP…");
    // backend always base64-encodes binary files
    const bin = atob(rawB64);
    const bytes = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
    zip = await JSZip.loadAsync(bytes.buffer);
  } catch (err) {
    toast("Failed to parse ZIP: " + err.message, "error");
    return;
  } finally {
    hideLoader();
  }
  ...
  zip.forEach((relPath, zipEntry) => {
    ...
    if (segs.some(s => SKIP.has(s))) return;
    entries.push({ rel: relPath, zipEntry });
  });

  if (!entries.length) {
    toast("ZIP is empty or has no eligible files", "error");
    return;
  }

  // Default target: same directory the ZIP lives in
  const zipDir  = zipPath.includes("/") ? zipPath.split("/").slice(0, -1).join("/") : "";
  const zipName = zipPath.split("/").pop();

  function buildPreview(base) {
    const prefix = base ? base.replace(/\/+$/, "") + "/" : "";
    return entries.slice(0, 6).map(it =>
      `<div style="font-family:monospace;font-size:11px;color:var(--text-dim);padding:1px 0">
        ${escapeHtml(prefix + it.rel)}
      </div>`
    ).join("") + (entries.length > 6
      ? `<div style="font-size:11px;color:var(--text-dim);padding:2px 0 0">…and ${entries.length - 6} more files</div>`
      : "");
  }
  ...
  openModal({
    ...
    bodyHtml: `
      <p style="margin:0 0 12px;color:var(--text-dim)">
        Found <b>${entries.length}</b> file${entries.length > 1 ? "s" : ""} inside
        <span style="font-family:monospace;color:var(--accent)">${escapeHtml(zipName)}</span>.
        Full folder structure will be preserved.
      </p>
      <div class="form-row">
        <label>Extract into directory <span style="font-weight:400;color:var(--text-dim)">(leave empty for repo root)</span></label>
        <input id="fBase" value="${escapeAttr(zipDir)}" placeholder="e.g. src/libs or leave empty" autocomplete="off" />
      </div>
      <div class="form-row">
        <label>Destination preview</label>
        <div id="unzipPreview" style="background:var(--surface-2,rgba(255,255,255,.04));border:1px solid var(--border);border-radius:6px;padding:8px 10px;min-height:32px;max-height:160px;overflow-y:auto">
          ${buildPreview(zipDir)}
        </div>
      </div>
      <div class="form-row">
        <label>Commit message</label>
        <input id="fBaseMsg" value="GITPUSH: unzip ${escapeAttr(zipName)}" />
      </div>
    `,
    footer: [
      { label: "Cancel", onClick: (close) => close() },
      {
        label: `Extract ${entries.length} file${entries.length > 1 ? "s" : ""}`,
        className: "primary",
        onClick: (close) => {
          const baseDir   = $("fBase").value.trim().replace(/^\/+|\/+$/g, "");
          const msgPrefix = $("fBaseMsg").value.trim() || `GITPUSH: unzip ${zipName}`;
          close();
          runZipUpload(entries, baseDir, msgPrefix);
        }
      },
  ...
  setTimeout(() => {
    const inp  = $("fBase");
    const prev = $("unzipPreview");
    if (!inp || !prev) return;
    inp.addEventListener("input", () => {
      prev.innerHTML = buildPreview(inp.value.trim().replace(/^\/+|\/+$/g, ""));
    });
  }, 30);
}
