...
async function loadInitialContext() {
  ...
  // Server lost repo context (restart?) — try reconnecting from localStorage
  const saved = localStorage.getItem("gitpush_repo");
  if (saved) {
    try {
      const res = await api("/change-repo", {
        method: "POST",
        body: JSON.stringify({ repo: saved }),
      }, { retries: 1 });
      state.repo = saved;
      localStorage.setItem("gitpush_repo", saved);
      refreshRepoPill();
      refreshInfo({ silent: true }).catch(() => {});
      actionListFiles("");
    } catch (_) {
      localStorage.removeItem("gitpush_repo");
    }
  }
}
    ...
    footer: [
      ...
      { label: "Connect", className: "primary", onClick: async (close) => {
          ...
          try {
            ...
            state.repo = repo;
            localStorage.setItem("gitpush_repo", repo);
            state.currentPath = "";
...
async function runFolderUpload(items, baseDir, msgPrefix) {
  ...
  // Batch size: max 15MB per request or 80 files, whichever comes first
  ...
  setPanel(`Uploading · ${total} file(s) — preparing…`, `
    <div class="folder-summary">
      ...
      ${cell("Progress", `<span id="pgCount">0 / ${total}</span>`)}
    </div>
  ...
  for (const it of items) {
    try {
      ...
      filePayload.push({ path: it.rel, content, size: it.file.size });
    } catch (_) {
      const logEl = $("pgLog");
      if (logEl) logEl.innerHTML += `<div style="color:var(--danger)">✕ Read failed: ${escapeHtml(it.rel)}</div>`;
    }
    readDone++;
    if ($("pgFill"))  $("pgFill").style.width = `${Math.round((readDone / total) * 40)}%`;
    if ($("pgStatus")) $("pgStatus").textContent = `Reading… ${readDone}/${total}`;
    if ($("pgCount"))  $("pgCount").textContent = `${readDone} / ${total}`;
  }
  ...
  for (const f of filePayload) {
    const approxBytes = f.content.length * 0.75; // base64 → raw
    if (batch.length > 0 && (batchBytes + approxBytes > MAX_BATCH_BYTES || batch.length >= MAX_BATCH_FILES)) {
      batches.push(batch);
      batch = []; batchBytes = 0;
    }
    batch.push(f);
  ...
  for (let bi = 0; bi < batches.length; bi++) {
    ...
    if ($("pgStatus")) $("pgStatus").textContent = `Pushing${batchLabel}… ${uploadedCount + b.length}/${total} files`;

    const commitMsg = batches.length > 1
      ? `${msgPrefix} — batch ${bi+1}/${batches.length} (${b.length} files)`
      : `${msgPrefix} (${b.length} files)`;

    try {
      const data = await api("/upload-folder-atomic", {
        method: "POST",
        body: JSON.stringify({
          files: b.map(f => ({ path: f.path, content: f.content })),
          base_dir: baseDir,
          message: commitMsg,
        }),
      }, { retries: 1, timeout: 120000 });

      uploadedCount += b.length;
      lastCommit = data.commit || "";
      const pct = 40 + Math.round((uploadedCount / total) * 60);
      if ($("pgFill"))  $("pgFill").style.width = `${pct}%`;
      if ($("pgCount")) $("pgCount").textContent = `${uploadedCount} / ${total}`;
      if (logEl) {
        b.forEach(f => {
          logEl.innerHTML += `<div><span style="color:var(--success)">✓</span> ${escapeHtml(f.path)}</div>`;
        });
        logEl.scrollTop = logEl.scrollHeight;
      }
    } catch (e) {
      if ($("pgFill"))   $("pgFill").style.width = "100%";
      if ($("pgStatus")) $("pgStatus").innerHTML = `<span style="color:var(--danger)">✕ Batch ${bi+1} failed: ${escapeHtml(e.message)}</span>`;
      toast(`Upload failed on batch ${bi+1}: ${e.message}`, "error");
      return;
    }
  }
  ...
  if ($("pgStatus")) $("pgStatus").innerHTML =
    `<span style="color:var(--success)">✓ All ${uploadedCount} file(s) uploaded — commit <b>${lastCommit.slice(0,7)}</b></span>`;

  toast(`✓ Uploaded ${uploadedCount} file(s) in ${batches.length} commit${batches.length > 1 ? "s" : ""}`, "success");
  refreshInfo({ silent: true }).catch(() => {});
