...
function actionUploadFolder() {
  ...
  // On mobile, webkitdirectory is not supported — fall back to
  // multi-file picker and inform the user to use drag & drop instead.
  const isMobile = /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
  if (isMobile) {
    // Use the normal filePicker but allow multiple so they can
    // pick individual files. Show a hint about drag & drop.
    toast("Mobile: select files individually. For full folder upload, use drag & drop on desktop.", "");
    const picker = $("filePicker");
    picker.value = "";
    picker.onchange = (e) => handleFilesPicked(e, false);
    picker.click();
