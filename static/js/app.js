  ...
  openModal({
    ...
    bodyHtml: `
      <div class="danger-banner">
        <b>This will wipe every file in <code>${FIXED_USERNAME}/${escapeHtml(state.repo)}</code></b><br>
        <span style="color:var(--text-dim);font-size:12px">
          Commit history is preserved, but the working tree will be emptied in a single commit.
          This cannot be undone from the UI.
        </span>
      </div>
      <div class="form-row" style="margin-top:14px">
        <label>Type <b>DELETE ALL</b> to confirm</label>
        <input id="confirmInput" placeholder="DELETE ALL" autocomplete="off">
      </div>
      <div class="form-row">
        <label>Commit message</label>
    ...
    footer: [
      { label: "Cancel", onClick: (close) => close() },
      { label: "Wipe Repository", className: "danger", onClick: async (close) => {
          const confirm = $("confirmInput").value.trim();
          if (confirm !== "DELETE ALL") return toast("Type DELETE ALL exactly to confirm", "error");
          const message = $("dMsg").value.trim() || "GITPUSH: full repository cleanup";
          try {
            showLoader("Wiping repository…");
