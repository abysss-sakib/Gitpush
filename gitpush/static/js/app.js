...
async function loadInitialContext() {
  ...
  try {
    ...
    // If client has a repo but server lost it (e.g. Render dyno restart wiped
    // the ephemeral .gitpush_ctx.json), silently restore the server context so
    // subsequent API calls work correctly from any worker.
    if (state.repo && !ctx.repo) {
      api("/change-repo", {
        method: "POST",
  ...
  if (state.repo) {
    // Always refresh the pill after context is resolved — even if refreshInfo
    // fails (token error, rate limit, etc.) we still want the pill to show
    // the correct "user / repo" label so it doesn't look disconnected.
    refreshInfo({ silent: true }).catch(() => {
      // refreshInfo failed but repo is still selected — update hero manually
      // so the title shows even without live GitHub data.
      renderHero(null);
      refreshRepoPill();
    });
    actionListFiles("");
...
/* ============================================================
   Single file upload — always streams via multipart/form-data.
   Memory-light on the server, real progress bar in the browser,
   and works for files up to GitHub's 100 MB single-blob limit.
   ============================================================ */
const STREAM_THRESHOLD = 2 * 1024 * 1024;          // > 2MB → multipart streaming
const HARD_GITHUB_LIMIT = 100 * 1024 * 1024;       // GitHub REST API limit
...
async function runSingleFileUpload(item, baseDir, msgPrefix) {
  ...
  if (item.file.size > HARD_GITHUB_LIMIT) {
    toast(`File is ${formatBytes(item.file.size)} — GitHub's REST API only accepts ≤100 MB. Use Git LFS for larger files.`, "error");
    return;
  }
  ...
  // Always use the streaming endpoint — even small files; it keeps things
  // simple and the server path is identical regardless of size.
  return uploadFileStreamXHR(item, fullPath, msgPrefix);
}

/* Multipart upload with REAL progress bar, using XMLHttpRequest because
   fetch() does not yet expose request upload progress everywhere. */
function uploadFileStreamXHR(item, fullPath, msgPrefix) {
  return new Promise((resolve) => {
    ...
    xhr.timeout = 15 * 60 * 1000;  // 15 minutes for very large files

    let lastPct = -1;
    xhr.upload.onprogress = (e) => {
      if (!e.lengthComputable) return;
      // Reserve last 12% for the server's own commit work.
    ...
    xhr.onerror   = () => finishXHRError(xhr, "Network error during upload — connection dropped or server unreachable.", resolve);
...
function finishXHR(xhr, fullPath, resolve) {
  ...
  if (ok) {
    ...
    if (!msg) {
      ...
      msg = snippet
        ? `Server error ${xhr.status}: ${snippet}`
        : `Upload failed (${xhr.status}) — server returned no body`;
    }
    if ($("pgFill"))   $("pgFill").style.width = "100%";
    if ($("pgStatus")) $("pgStatus").innerHTML =
    ...
    if (data.trace && $("pgLog")) {
      $("pgLog").innerHTML =
        `<details open style="margin-top:6px"><summary style="cursor:pointer;color:var(--text-dim)">Server traceback (tap to copy)</summary>` +
        `<pre style="white-space:pre-wrap;font-size:11px;background:rgba(255,0,0,.06);padding:8px;border-radius:6px;margin:6px 0;max-height:240px;overflow:auto">${escapeHtml(data.trace)}</pre>` +
        `</details>`;
    }
    toast(msg, "error");
  }
...
/* ============================================================
   Folder upload.
     • If every file is small (< STREAM_THRESHOLD), use the original
       fast path: one JSON POST that creates blobs + tree + commit.
     • Otherwise, stream each file individually as multipart blobs,
       then commit them all atomically in ONE commit. This is what
       lets folders containing big files succeed on small dynos.
   ============================================================ */
async function runFolderUpload(items, baseDir, msgPrefix) {
  const total = items.length;

  // Reject anything bigger than GitHub will ever accept up front.
  const tooBig = items.find(it => it.file.size > HARD_GITHUB_LIMIT);
  if (tooBig) {
    toast(`"${tooBig.rel}" is ${formatBytes(tooBig.file.size)} — GitHub's REST API only accepts ≤100 MB per file.`, "error");
    return;
  }
  ...
  if (hasLarge) {
    return runFolderUploadStream(items, baseDir, msgPrefix, totalBytes);
  }
  return runFolderUploadInline(items, baseDir, msgPrefix);
}

/* Inline path — for folders made entirely of small files */
...
/* Streaming path — for folders that contain at least one big file.
   Uploads each file as a multipart blob (memory-light on the server),
   collects the SHAs, then issues ONE atomic commit-tree call. */
async function runFolderUploadStream(items, baseDir, msgPrefix, totalBytes) {
  ...
  for (const it of items) {
    ...
    if ($("pgStatus")) $("pgStatus").textContent =
      `Uploading ${i}/${total}: ${it.rel} (${sizeStr})…`;
    ...
    try {
      const sha = await uploadBlobStreamXHR(it, (sent) => {
        const cur = bytesSentSoFar + sent;
        const pct = Math.round((cur / totalBytes) * 88);
        if ($("pgFill")) $("pgFill").style.width = pct + "%";
      });
      blobs.push({ path: it.rel, sha });
      bytesSentSoFar += it.file.size;
      const line = $(lineId);
      if (line) line.innerHTML = `<span style="color:var(--success)">✓</span> ${escapeHtml(it.rel)} <span class="muted">(${sha.slice(0,7)})</span>`;
    } catch (e) {
      const line = $(lineId);
      if (line) line.innerHTML = `<span style="color:var(--danger)">✕</span> ${escapeHtml(it.rel)} — ${escapeHtml(e.message || "failed")}`;
      // Stop on first error so we don't half-commit a folder.
      toast(`Upload failed at ${it.rel}: ${e.message || "error"}`, "error");
      if ($("pgStatus")) $("pgStatus").innerHTML =
        `<span style="color:var(--danger)">✕ Stopped after ${i - 1}/${total}.</span>`;
      return;
    }
  }
...
/* Single-blob multipart upload that resolves with the blob SHA. */
