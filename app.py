...
def _fresh_repo():
    ...
    if not username:
        raise LookupError(
            ...
            "Click 'Change User' in the navbar to set one.")
    if not repo_name:
        raise LookupError(
            "No repository selected. "
            "Click 'Change Repo' in the navbar and pick a repo first.")
...
def handle_github(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            # 400 (not 404) so the browser doesn't confuse a missing repo
            # selection with a missing route.
            return api_error(str(e), 400, code="no_repo_selected")
...
# Bump this whenever a meaningful change is shipped — gives a quick way to
# verify which build is actually deployed by visiting /health.
BUILD_VERSION = "streaming-v2 (2026-04-28)"
...
def health():
    ...
    return api_ok(
        ...
        endpoints=sorted({
            ...
            if str(r).startswith("/") and not str(r).startswith("/static")
        }),
    )
...
# ---------------------------------------------------------------------------
# Streaming upload — multipart/form-data, no JSON base64 inflation
# Handles big files on small dynos (Render free 512MB)
# ---------------------------------------------------------------------------
GITHUB_MAX_BLOB = 100 * 1024 * 1024            # GitHub REST API hard limit
USE_GITDATA_THRESHOLD = 1 * 1024 * 1024        # >1MB → Git Data API path


def _upload_bytes_to_repo(repo, raw, path, message, branch, overwrite=True):
    """Common write path used by /upload-file-stream. Returns api_ok payload."""
    size = len(raw)
    if size > GITHUB_MAX_BLOB:
        return api_error(
            f"File is too large for GitHub's REST API "
            f"({size/1024/1024:.1f}MB > 100MB). "
            f"Files larger than 100MB require Git LFS.", 413)

    # Detect existing file (so we can decide create vs update)
    existing = None
    try:
        existing = repo.get_contents(path, ref=branch)
        if isinstance(existing, list):
    ...
    if size > USE_GITDATA_THRESHOLD:
        # Git Data API: blob → tree → commit → ref. Works for any binary.
        log.info("  → encoding blob (%d bytes)", size)
        b64 = base64.b64encode(raw).decode("ascii")
        del raw
        log.info("  → create_git_blob (%d b64 chars)", len(b64))
        blob = _retry_github(lambda: repo.create_git_blob(b64, "base64"))
        del b64
        log.info("  → blob created: %s", blob.sha[:7])
        ...
        log.info("  done: %s %s (%s)", action, path, new_commit.sha[:7])
        return api_ok(
            action=action,
            message=f"{action.capitalize()}d {path}",
    ...
    # Small file → simple Contents API (single HTTP call to GitHub).
    if existing:
        result = _retry_github(lambda: repo.update_file(
            path=path, message=message, content=raw,
            sha=existing.sha, branch=branch))
...
@app.route("/upload-file-stream", methods=["POST"])
@handle_github
def upload_file_stream():
    """
    Memory-efficient upload using multipart/form-data.
    Avoids the JSON-base64 inflation that breaks large files (~30MB+) on
    small dynos.
    ...
    Form fields:
      ...
      branch     - optional branch (defaults to repo's default_branch)
      overwrite  - "1"/"0" (default "1")
    """
    repo = get_current_repo()
    log.info("upload_file_stream: repo=%s", repo.full_name)

    # Wrap multipart access in try so a malformed body becomes a clean error
    # instead of bubbling up to Werkzeug's default HTML 500 page.
    try:
        f = request.files.get("file")
    except Exception as e:
        log.exception("multipart parse failed")
        return api_error(
            f"Could not parse uploaded file ({type(e).__name__}): {e}", 400)

    if not f:
        return api_error(
            "No file uploaded — multipart 'file' field is required.", 400)

    path      = (request.form.get("path") or "").strip()
    message   = (request.form.get("message") or f"GITPUSH: upload {path}").strip()
    branch    = (request.form.get("branch") or "").strip() or repo.default_branch
    overwrite = request.form.get("overwrite", "1") != "0"

    if not _is_safe_path(path):
        return api_error("Invalid file path", 400)

    # Early size check via Content-Length before reading into memory.
    # Werkzeug enforces MAX_CONTENT_LENGTH (500 MB) at the WSGI layer, but
    # on Render free dynos the 512 MB RAM limit is hit first, causing a raw
    # 500. Checking here lets us return a clean 413 with a helpful message.
    content_length = request.content_length
    if content_length and content_length > GITHUB_MAX_BLOB:
        return api_error(
            f"File is too large for GitHub's REST API "
            f"({content_length/1024/1024:.1f}MB > 100MB). "
            f"Files larger than 100MB require Git LFS.", 413)

    # Warn if the file might be large enough to OOM on Render free (512 MB).
    DYNO_SAFE_LIMIT = 80 * 1024 * 1024   # ~80 MB: leaves room for base64 inflation
    if content_length and content_length > DYNO_SAFE_LIMIT:
        log.warning(
            "  large file upload: %d MB — risk of OOM on free dyno",
            content_length // 1024 // 1024)

    log.info("  reading uploaded file from spool…")
    try:
        raw = f.read()
    except MemoryError:
        log.exception("MemoryError while reading uploaded file")
        return api_error(
            "Server ran out of memory while reading the upload. "
            "This Render free dyno only has ~512MB; try a smaller file or "
            "upgrade the dyno.", 507)
    log.info("  read %d bytes for path=%s branch=%s", len(raw), path, branch)

    try:
        return _upload_bytes_to_repo(repo, raw, path, message, branch, overwrite)
    except MemoryError:
        log.exception("MemoryError while pushing to GitHub")
        return api_error(
            "Server ran out of memory while pushing to GitHub. "
            "Free dyno is ~512MB; this file is too large for it.", 507)


@app.route("/create-blob-stream", methods=["POST"])
@handle_github
def create_blob_stream():
    """
    Multipart blob upload. Returns just the blob SHA so the client can
    later combine many blobs into one atomic commit via /commit-tree.

    Used by the folder-upload flow to handle large files without inflating
    ...
    try:
        ...
        log.exception("MemoryError reading file")
        return api_error(
            "Server ran out of memory reading the file (free dyno ~512MB).",
            507)
    size = len(raw)
    if size > GITHUB_MAX_BLOB:
        return api_error(
            f"File is too large for GitHub's REST API "
            f"({size/1024/1024:.1f}MB > 100MB).", 413)
    ...
    try:
        ...
        log.exception("MemoryError while sending blob to GitHub")
        return api_error(
            "Server ran out of memory while sending the file to GitHub "
            "(free dyno ~512MB).", 507)
    log.info("create_blob_stream: %d bytes → %s", size, blob.sha[:7])
    return api_ok(sha=blob.sha, size=size)


@app.route("/commit-tree", methods=["POST"])
@handle_github
def commit_tree():
    """
    Atomically commit a list of pre-uploaded blobs into a single commit.

    Body:
      blobs:   [{ "path": "rel/path.ext", "sha": "<blob-sha>" }, ...]
      base_dir: optional prefix prepended to each blob path
      branch:  optional branch (defaults to repo's default_branch)
    ...
    branch   = body.get("branch") or repo.default_branch
    message  = (body.get("message") or "GITPUSH: upload").strip()

    if not isinstance(blobs, list) or not blobs:
        return api_error("`blobs` must be a non-empty list", 400)
    ...
    for b in blobs:
        ...
        if not sha or len(sha) < 6:
            return api_error(f"Invalid blob sha for {full!r}", 400)
        elements.append(InputGitTreeElement(
            path=full, mode="100644", type="blob", sha=sha))

    def commit_once():
        log.info("commit_tree: %d entries on heads/%s", len(elements), branch)
        ref         = repo.get_git_ref(f"heads/{branch}")
        base_commit = repo.get_git_commit(ref.object.sha)
        base_tree   = base_commit.tree
    ...
    new_commit = _retry_github(commit_once)
    log.info("commit_tree: committed → %s", new_commit.sha[:7])
    return api_ok(
        action="commit_tree",
        commit=new_commit.sha,
        files_count=len(elements),
        branch=branch,
        message=f"Committed {len(elements)} file(s) in 1 commit",
    )
...
_API_PREFIXES = (
    ...
    "/upload-file-stream", "/create-blob-stream", "/commit-tree",
...
def server_error(_):
    ...
    return _safe_error_response(500, "Internal server error")


# ---- universal catch-all -------------------------------------------------
# Whatever exception escapes the view (memory, timeout, multipart parse
# failure, etc.) ends up here and is returned as JSON so the browser can
# show a real diagnostic instead of Werkzeug's plain HTML 500 page.
def _safe_error_response(status, fallback_msg, exc=None):
    """Always return a JSON response, even if jsonify itself fails."""
    payload = {"ok": False, "error": fallback_msg}
    if exc is not None:
        payload["error"] = (
            f"{fallback_msg} ({type(exc).__name__}): {exc}"
        )[:600]
        # last 1500 chars of traceback — usually enough to find the cause
        try:
            payload["trace"] = "".join(
    ...
    try:
        ...
        # Hard fallback — if jsonify itself blew up, at least return something
        # the client can parse with JSON.parse().
        body = json.dumps({"ok": False, "error": fallback_msg})
...
def http_exception(e):
    # Things like 404, 405, 413 already pick their own handlers above. This
    # is a safety net for any other HTTPException so they come back as JSON
    # rather than HTML.
    log.warning("HTTPException %s: %s", e.code, e.description)
    return _safe_error_response(
        e.code or 500, e.description or "HTTP error", e)


@app.errorhandler(Exception)
def unhandled_exception(e):
    # Last-resort handler for anything else (RuntimeError, MemoryError,
    # multipart parse errors, etc.). Logs full traceback to Render and
    # returns a JSON body the UI can read.
    log.exception("Unhandled exception")
    return _safe_error_response(500, "Server crashed while handling request", e)
