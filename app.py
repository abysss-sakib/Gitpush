...
def upload_folder_atomic():
    ...
    for f in files:
        ...
        # GitHub API rejects blobs > 100MB
        if len(raw_bytes) > 100 * 1024 * 1024:
            return api_error(f"File too large for GitHub API (max 100MB): {full}", 400)
        # Use text mode for text files (faster), blob for binary
        try:
            text_content = raw_bytes.decode("utf-8")
            blob = _retry_github(lambda tc=text_content: repo.create_git_blob(tc, "utf-8"))
        except UnicodeDecodeError:
            blob = _retry_github(lambda rb=raw_bytes: repo.create_git_blob(
                base64.b64encode(rb).decode("ascii"), "base64"))
